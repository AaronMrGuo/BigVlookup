...
def get_spe_sal(data_path):
    ...
    spe_name={'广西桂林汇通药业有限公司': '销售流向，自采数据，可能多流向',
             ...
             '十堰市君琪安药业有限公司':'销售流向，自采数据，可能多流向',
             '芜湖上药医药有限公司': '销售流向:原始数据错列，导致清洗数据产品名称缺失',
             '宁海县医药药材有限责任公司':'数据可能错列，产品名称 规格 生产厂家 在一起',
              }
    new_data_path = data_path + '处理成功'
    for dirpath, dirname, filenames in os.walk(new_data_path):
        for fname in filenames:
                ...
                if '销售日期' in list(df.columns):
                    ...
                    for i in df.columns:
                        if i == '日期' or i=='出库日期 ' or i == '释放日期' or i == '出库日期' or i == '流向日期' or i == '销售时间' or i == '出入库日期' or i == '发货日期'\
                            ...
                            or i=='财务审核时间' or i=='购买日期' or i=='开单日期(含时分秒' or i=='凭证年度' or i=='订单日期' or i=='出库退库日期'\
                            or i=='开单时间' or i=='结算日期' or i=='销售确认日期' or i=='订单审核日期' or i=='业务开票':
...
def get_product_name(data_path):
    ...
    new_data_path = data_path + '处理成功'
    product_name_list=[]
    with open(r'C:\Users\guodingyu\Desktop\Guodingyu\Study\NLP\clean_data3.0\features\shxy\product_name.txt') as f:
...
def get_product_spe(data_path):
    ...
    new_data_path = data_path + '处理成功'
    product_spe_list=[]
    with open(r'C:\Users\guodingyu\Desktop\Guodingyu\Study\NLP\clean_data3.0\features\shxy\product_spec.txt') as f:
...
def get_product_manu(data_path):
    ...
    print('='*200)
    new_data_path = data_path + '处理成功'
    with open(r'C:\Users\guodingyu\Desktop\Guodingyu\Study\NLP\clean_data3.0\features\shxy\factory_manu.txt') as f:
...
def get_batch_num(data_path):
    ...
    new_data_path = data_path + '处理成功'
    batch_num_list=[]
    with open(r'C:\Users\guodingyu\Desktop\Guodingyu\Study\NLP\clean_data3.0\features\shxy\batch_num.txt') as f:
                ...
                if df.shape[0]>0:
                    for i in list1:
                        if i not in list(df.columns):
                            new_floder = data_path + '处理失败' + '\\' + str(i) + '\\' + '\\'.join(file_path.split('\\')[-2:-1])
...
def check_data(data_path):
    ...
    for i in column_name:
        concat_df[i] = ''
    concat_df = concat_df[['销售日期','客户代码', '客户名称', '产品编码', '产品名称', '规格', '产品单位', '数量', '批号', '单价', '金额', '送货地址', '生产厂家','效期', '经销商' ]]
...
def shxy_control_program():
    ...
    data_path=r'D:\shxy\接收文件二级商'
    copy_path = r'C:\Users\guodingyu\Desktop\Guodingyu\Work\Project\xy\2.xy分拣\10月\二级商\备份'  # 原始文件备份文件夹
    # data_path=r'C:\Users\guodingyu\Downloads\接收文件二级商'
    # copy_path=r'C:\Users\guodingyu\Downloads\备份\二级商'#原始文件备份文件夹
    ...
    if len(floder_list)==0:
        msg='N'
        print('='*74+f' {str(datetime.datetime.now())[:19]} xy数据分拣系统 实时监控 无数据  '+'='*73)
        print('-'*200)
        time.sleep(180)
        shxy_control_program()
    else:
        msg='Y'
        print(f'{str(datetime.datetime.now())[:19]} xy数据分拣系统 实时监控 来数据了...')
        print('=' * 200)
        for floder in os.listdir(now_path):
...
def sal_step1(data_path):
    ...
    del_temporary(data_path)  #删除~开头临时文件
